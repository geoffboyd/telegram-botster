module.exports = {
  ...
  execute(bot, channel, from, text, commandNames) {
    ...
    args.shift();
    if (!args[0]) { return bot.telegram.sendMessage(channel, `You rolled a ${Math.ceil(Math.random()*numberOfSides)}`); }
    ...
    if (isNaN(args[0]) || Math.floor(args[0]) < 3) { return bot.telegram.sendMessage(channel, `Usage: \`${prefix}roll\` or \`${prefix}roll 20\` or \`${prefix}roll 3d12\``); }
    ...
    return bot.telegram.sendMessage(channel, `Your total is: ${total}\nYour dice were: ${dice.join(', ')}`);
  },
};
